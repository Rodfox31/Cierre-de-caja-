...
function BillsPanel({ billEntries, updateRowTotal, finalTotal }) {
    return (
        <Paper elevation={2} sx={{ p: 0.7, borderRadius: 4, width: '100%', minHeight: 0 }}>
            <Typography variant="h6" gutterBottom>Detalle de Efectivo</Typography>
            <Stack spacing={0.15}>
                <Grid container spacing={1} sx={{ alignItems: 'center', color: 'text.secondary', px: 0.5 }}>
                ...
                {billEntries.map((bill, index) => (
                    <React.Fragment key={index}>
                        <Grid container spacing={1} sx={{ alignItems: 'center', py: 0.05, px: 0.5, minHeight: 0 }}>
                            <Grid item xs={5}><Typography variant="body1">{bill.label}</Typography></Grid>
                            <Grid item xs={3}>
                                <TextField type="number" size="small" variant="outlined" placeholder="0" value={bill.cantidad || ""}
                                    onChange={(e) => updateRowTotal(index, e.target.value)} sx={{ width: '100%' }}
                                    inputProps={{ min: 0, 'aria-label': `Cantidad de ${bill.label}` }}
                                />
                            </Grid>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', px: 0.5 }}>
                    ...
                    <Typography variant="body1" fontWeight="bold">{formatCurrency(-10000)}</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', color: 'primary.main', px: 0.5, mt: 0.5 }}>
                    <Typography variant="h6" fontWeight="bold">Cash Total:</Typography>
                    <Typography variant="h6" fontWeight="bold">{formatCurrency(finalTotal)}</Typography>
                </Box>
            </Stack>
        </Paper>
    );
    ...
    return (
        <Paper elevation={2} sx={{ p: 0.7, borderRadius: 4, width: '100%', minHeight: 0 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                <Typography variant="h6">Depósitos (Brinks)</Typography>
                <IconButton onClick={addNewRow} color="primary" title="Agregar nueva fila" aria-label="Agregar nueva fila">
                    <AddCircleOutlineIcon />
                </IconButton>
            </Box>
            <Stack spacing={0.1}>
                <Grid container spacing={1} sx={{ color: 'text.secondary', textAlign: 'left' }}>
                ...
                {brinksEntries.map((entry, idx) => (
                    <React.Fragment key={idx}>
                        <Grid container spacing={1} alignItems="center" sx={{ minHeight: 0 }}>
                            <Grid item xs={7} sx={{ display: 'flex', alignItems: 'center', pl: 0.5 }}>
                                <TextField fullWidth size="small" placeholder="Código de depósito" value={entry.codigo || ""} onChange={(e) => updateBrinksRow(idx, "codigo", e.target.value)} inputProps={{ style: { textAlign: 'left' }, 'aria-label': `Código depósito ${idx+1}` }} sx={{ ml: '-8px' }} />
                            </Grid>
                            <Grid item xs={5} sx={{ display: 'flex', alignItems: 'center', pl: 0.5 }}>
                                <TextField fullWidth size="small" type="number" placeholder="0" value={entry.monto || ""} onChange={(e) => updateBrinksRow(idx, "monto", e.target.value)} inputProps={{ style: { textAlign: 'left' }, min: 0, 'aria-label': `Monto depósito ${idx+1}` }} sx={{ ml: '-8px' }} />
                            </Grid>
                        </Grid>
                        {idx < brinksEntries.length - 1 && <Divider light />}
                    </React.Fragment>
                ))}
            </Stack>
            <Divider sx={{ my: 0.5 }} />
            <Box sx={{ display: 'flex', justifyContent: 'space-between', color: 'primary.main' }}>
                <Typography variant="h6" fontWeight="bold">Total Depósitos:</Typography>
...
function PaymentMethodsPanel({ medios_pago, paymentEntries, setPaymentEntries, dynamicEfectivo }) {
    ...
    const updatePaymentRow = (index, field, value) => {
        ...
        let facturadoVal;
        if (field === 'facturado' && index !== 0) {
          facturadoVal = parseSumExpression(value || '0');
        } else {
          facturadoVal = parseCurrency(newEntries[index].facturado || '0');
        }
    ...
    return(
        <Paper elevation={2} sx={{ p: 1.2, borderRadius: 4, width: '100%', minHeight: 0 }}>
            <Typography variant="h6" gutterBottom>Medios de Pago</Typography>
            <Grid container spacing={2} sx={{ mb: 1, color: 'text.secondary', px: 1 }}>
            ...
            {(!medios_pago || medios_pago.length === 0) ? (
                ...
                paymentEntries.map((entry, idx) => (
                    <Grid container spacing={2} key={idx} alignItems="center" sx={{ py: 0.05, px: 1, borderRadius: 1 }}>
                        <Grid item xs={4}><Typography variant="body1">{entry.medio}</Typography></Grid>
                        <Grid item xs={2.5}>
                            <TextField fullWidth size="small" placeholder="Facturado" value={entry.facturado}
                         ...
                         <Grid item xs={2.5}>
                            <TextField fullWidth size="small" placeholder="Cobrado" value={entry.cobrado}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant="h6" fontWeight="bold">Diferencia Total:</Typography>
                <Typography variant="h6" fontWeight="bold" sx={{ color: grandTotal === 0 ? 'text.primary' : (grandTotal > 0 ? 'success.main' : 'error.main') }}>
...
function JustificacionesPanel({ paymentEntries, ajustesMotivos, fecha, onSumChange, onJustificacionesChange }) {
    ...
    return (
        <Paper elevation={2} sx={{ p: 1.2, borderRadius: 4, width: '100%', minHeight: 0 }}>
            <Typography variant="h6" gutterBottom>Justificaciones de Diferencias</Typography>
            {rowsWithDifference.length > 0 || justificaciones.length > 0 ? (
                <Stack spacing={1.2}>
                    <Grid container spacing={1} sx={{ color: 'text.secondary', textAlign: 'center', px: 1 }}>
                    ...
                    {justificaciones.map((just, idx) => (
                        <Grid container spacing={1} key={idx} alignItems="center" sx={{
                            border: 1, borderColor: 'divider', borderRadius: 2, p: 1, minHeight: 0,
                        }}>
                    ...
                    <Box sx={{ width: '100%', mt: 1, mb: 0.5 }}>
                        <Typography variant="subtitle2" sx={{ mb: 0.5 }}>Agregar Justificación Manual</Typography>
                    </Box>
                    <Grid container spacing={1} alignItems="center" sx={{ border: 1, borderColor: 'divider', borderRadius: 2, p: 0.5, bgcolor: 'background.paper', minHeight: 0 }}>
                        <Grid item xs={1.5}>
                        ...
                        <Grid item xs={12} sx={{ textAlign: 'right', mt: 0 }}>
                            <Button variant="outlined" color="primary" size="small" onClick={agregarJustificacionManual} sx={{ minHeight: 0, py: 0.5 }}>
...
function FinalizationPanel({
    ...
    return (
        <Paper elevation={2} sx={{ p: 1.2, borderRadius: 4, minHeight: 0 }}>
            <Stack spacing={1.2}>
                <Typography variant="h6">Finalizar Cierre</Typography>
